...
def main(root, counter):
    ...
    for img_id in img_ids:
        ...
        if img is not None:
            ...
            unique = unique = np.unique(anns_img)
            ...
            save_pth_img =os.path.join('final-dataset-test/image', f'{str(counter).zfill(6)}.jpg')
            save_pth_mask = os.path.join('final-dataset-test/mask', f'{str(counter).zfill(6)}.tif')
...
if __name__ == "__main__":
    ...
    cocodict = {}
    root = "replaced-batches/batch_11"
    alldirs = os.walk(root)

    # print(sum(1 for _ in alldirs))
